...
// Funkcija za obradu dela podataka
void processRange(const std::vector<int>::iterator begin, const std::vector<int>::iterator end, std::atomic<double>& total_sum, std::mutex& mtx) {
    double local_sum = 0;
    for (auto it = begin; it != end; ++it) {
        local_sum += std::sqrt(*it);  // Primer obrade: kvadratni koren svakog elementa
    }
